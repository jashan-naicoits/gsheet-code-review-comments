...
class GoogleSpreadsheetWorksheet {
  ...
  // Setting header row props

  _headerX = 'A';
  _headerY = 1;

  async setHeaderProps(headerX, headerY) {
    this._headerX = headerX;
  ...
  async loadHeaderRow() {
    const rows = await this.getCellsInRange(`${this._headerX}${this._headerY}:${this.lastColumnLetter}${this._headerY}`);
    if (!rows) {
  ...
  async addRows(rows, options = {}) {
    ...
    const response = await this._spreadsheet.axios.request({
      method: 'post',
      url: `/values/${this.encodedA1SheetName}!${this._headerX}${this._headerY}:append`,
      params: {
